...
//BackgroundImage
  ...
  $ext1=getExt($name);
  $target1="background/".time().".".$ext1;

   if($ext1!="jpg" && $ext1!="JPG"){
	   echo "Invalid file type";
      }
else if(file_exists($target1)){
	echo "File already exists";
}
else{
	$conn = OpenCon();
	$t1= move_uploaded_file($tmp,$target1);
	//$sql="insert into picture values('','".$target."','".$user."')";
	$sql="INSERT INTO `background`(`id`, `bimage`, `username`) VALUES ('','$target1','$user')";
	//echo $sql;
	$result = $conn->query($sql);
	
	echo "File uploaded:".$target1;
	CloseCon($conn);
}
...
global $data1;
		   $conn = mysqli_connect("localhost", "root", "","onlineticket");
            $sql='SELECT background.bimage  FROM background INNER JOIN user ON background.username = user.username ';
           $result = mysqli_query($conn, $sql)or die(mysqli_error($conn));
            $data1=array();
	         while($row = $result->fetch_assoc()) {
				 $data1[]=$row;
				if(file_exists($row["bimage"]))
		{
			//echo "sabir";
	        $b="<img alt='not found' src='".$row["bimage"]."' />";
...
do {
    ...
    background-image: url('$b');
			 ...
			 echo "    body{
	    background-image:url('<?php echo $b; ?>');
...
echo "</style>";
	...
	background-image: url('C:\xampp\htdocs\OnlineTicketGenerator\background\$b');
}";
echo "</style>";
	...
	<div class="container">
		...
		<div class="col-sm-4">
			...
			<div class="form-group">
				 ...
				 <label for="LOGO">Insert Your Background Image</label><br>
